...
from config import INCOMING_FUEL, BRONZE_TRANSACTION, ARCHIVE_SCHEMA_FUEL, METADATA_FUEL
...
spark = get_spark("Fuel Transaction Bronze Ingestion")

REQUIRED_COLUMNS = ["transaction_id", "vin", "fuel_liters", "odometer_reading", "timestamp"]
...
def load_processed_manifest():
    if not path_exists(METADATA_FUEL):
        return None
    try:
        return spark.read.parquet(METADATA_FUEL)
    except Exception:
        return None
...
def archive_invalid_file(file_path, reason):
    move_file(file_path, ARCHIVE_SCHEMA_FUEL)
...
def append_manifest_records(records):
    if not records:
        return
    ensure_directory(METADATA_FUEL)
    manifest_df = spark.createDataFrame(records)
    manifest_df.write.mode("append").parquet(METADATA_FUEL)


def fuel_transaction_bronze():
    try:
        ensure_directory(BRONZE_TRANSACTION)
        ensure_directory(ARCHIVE_SCHEMA_FUEL)
        ensure_directory(METADATA_FUEL)

        # ====================================
        # 1. Validate incoming files one by one
        # ====================================
        incoming_files = list_csv_files(INCOMING_FUEL)
        ...
        df = df \
            ...
            .withColumn("_row_hash", md5(concat_ws("||", 
                df["transaction_id"], df["vin"], df["timestamp"]
            )))
        ...
        if DeltaTable.isDeltaTable(spark, BRONZE_TRANSACTION):
            try:
                existing_today_hashes = spark.read.format("delta").load(BRONZE_TRANSACTION).filter(
        ...
        if append_count > 0:
            df_to_append.write \
                ...
                .save(BRONZE_TRANSACTION)
            logger.info(f"✅ Appended {append_count} new rows to bronze")
            
            # Maintenance
            DeltaTable.forPath(spark, BRONZE_TRANSACTION).optimize().executeCompaction()
            DeltaTable.forPath(spark, BRONZE_TRANSACTION).vacuum(168.0)
        else:
        ...
        logger.error(f"❌ Error in fuel_transaction_bronze: {str(e)}")
        raise

if __name__ == "__main__":
    fuel_transaction_bronze()
