...
from pyspark.sql.functions import current_timestamp, current_date, col, md5, concat_ws, lit
import logging
...
from config import INCOMING_REGISTRY, BRONZE_REGISTRY, ARCHIVE_SCHEMA_REGISTRY, METADATA_REGISTRY
from delta.tables import DeltaTable

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

spark = get_spark("Registry Bronze Ingestion")

REQUIRED_COLUMNS = ["vin", "model", "mfg_year", "fuel_type"]
...
def load_processed_manifest():
    if not path_exists(METADATA_REGISTRY):
        return None
    try:
        return spark.read.parquet(METADATA_REGISTRY)
    except Exception:
        return None
...
def archive_invalid_file(file_path, reason):
    move_file(file_path, ARCHIVE_SCHEMA_REGISTRY)
...
def append_manifest_records(records):
    if not records:
        return
    ensure_directory(METADATA_REGISTRY)
    manifest_df = spark.createDataFrame(records)
    manifest_df.write.mode("append").parquet(METADATA_REGISTRY)


def registry_bronze():
    try:
        ensure_directory(BRONZE_REGISTRY)
        ensure_directory(ARCHIVE_SCHEMA_REGISTRY)
        ensure_directory(METADATA_REGISTRY)

        # ====================================
        # 1. Validate incoming files one by one
        # ====================================
        incoming_files = list_csv_files(INCOMING_REGISTRY)
        ...
        df = df \
            ...
            .withColumn("_row_hash", md5(concat_ws("||", 
                df["vin"], df["model"], df["mfg_year"], df["fuel_type"]
            )))
        ...
        null_models = df.filter(col("model").isNull()).count()
        null_fuels = df.filter(col("fuel_type").isNull()).count()
        
        if null_vins > 0:
            logger.warning(f"⚠️  {null_vins} records with null VIN")
        if null_models > 0:
            logger.warning(f"⚠️  {null_models} records with null model")
        if null_fuels > 0:
            logger.warning(f"⚠️  {null_fuels} records with null fuel_type")
        ...
        if DeltaTable.isDeltaTable(spark, BRONZE_REGISTRY):
            try:
                existing_today_hashes = spark.read.format("delta").load(BRONZE_REGISTRY).filter(
        ...
        if append_count > 0:
            df_to_append.write \
                ...
                .save(BRONZE_REGISTRY)
            logger.info(f"✅ Appended {append_count} new rows to bronze")
            
            # Maintenance
            DeltaTable.forPath(spark, BRONZE_REGISTRY).optimize().executeCompaction()
            DeltaTable.forPath(spark, BRONZE_REGISTRY).vacuum(168.0)
        else:
        ...
        print(f"✅ Null fuel types: {null_fuels}")
        print(f"✅ Registry Bronze Ingested")
        
    except Exception as e:
        logger.error(f"❌ Error in registry_bronze: {str(e)}")
        raise
