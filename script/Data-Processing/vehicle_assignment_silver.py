...
def assignment_silver():
    ...
    # -----------------------------
    # 0. Precondition: Verify upstream tables exist
    # -----------------------------
    ...
    # Normalize business keys and attributes for stable matching.
    df = df \
    ...
    # Filter invalid rows before SCD2 logic.
    valid_condition = (
    ...
    # Referential integrity: only keep VINs present in registry silver.
    valid_df = valid_df.join(registry_df, on="vin", how="inner")

    # Archive invalid/rejected rows for inspection.
    ...
    if invalid_count > 0:
        ...
        logger.warning(f"⚠️  Archived {invalid_count} invalid assignment rows")

    after_clean_count = valid_df.count()
    dropped_count = before_clean_count - after_clean_count
    if dropped_count > 0:
        logger.warning(f"⚠️  Dropped {dropped_count} invalid assignment rows during silver cleaning")

    # -----------------------------
    # 3. Deduplicate (highest daily_rate)
    # -----------------------------
    ...
    # Driver-level conflict handling: one active record per driver per start_date window.
    ...
    if rejected_driver_conflict_count > 0:
        ...
        logger.warning(f"⚠️  Archived driver assignment conflicts: {rejected_driver_conflict_count}")

    df = driver_conflict_df.filter(col("rn_driver") == 1).drop("rn_driver")

    # -----------------------------
    # 4. Sort records per VIN
    # -----------------------------
    ...
    # Continuity: end_date cannot be earlier than start_date.
    ...
    if rate_check > 0:
        logger.warning(f"⚠️  {rate_check} records with invalid (<=0) daily_rate")
    
    final_count = final_df.count()
    logger.info(f"Final assignment records: {final_count}")

    # ====================================
    # 9. Optimize Partitioning & Merge Silver
    # ====================================
