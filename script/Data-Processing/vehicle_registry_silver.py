...
def registry_silver():
    ...
    # Step 1: Load raw vehicle registry data from the Bronze Delta table.
    df = spark.read.format("delta").load(BRONZE_PATH)

    # To process efficiently, we only look at the most recently ingested batch of data.
    latest_date = df.selectExpr("max(ingestion_date)").collect()[0][0]
    df = df.filter(col("ingestion_date") == latest_date)

    # Step 2: Standardize data formats (e.g., uppercase VINs and fuel types, trim whitespace).
    ...
    # Apply our core data quality rules: drop records with missing keys, invalid VIN lengths, or manufacturing years in the future.
    ...
    if invalid_core_df.count() > 0:
        logger.warning(
            f"Invalid registry rows dropped: {invalid_core_df.count()} "
    ...
    # Step 2b: Handle irreconcilable conflicts.
    # If a single VIN is registered as two completely different models in the same year, we quarantine both records for manual review.
    ...
    if conflict_count > 0:
        ...
        logger.warning(f"Archived {conflict_count} conflicting registry rows")

    df = valid_df.join(conflict_keys, on=["vin", "mfg_year"], how="left_anti")

    if dropped_count > 0:
        logger.warning(f"Dropped {dropped_count} invalid registry rows during silver cleaning")

    # Step 3: Deduplicate identical VINs.
    # If a VIN appears multiple times (e.g., an update to the record), we keep the version with the most recent manufacturing year.
    ...
    # Step 4: Perform a final sanity check for null values before writing to the Silver layer.
    ...
    # Step 5: Optimize data layout and upsert.
    # We repartition by VIN to balance file sizes and partition the final table by manufacturing year.
    ...
    # Regular Delta Lake maintenance: apply Z-Ordering by VIN to accelerate future joins and vacuum old files.
